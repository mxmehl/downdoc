...
module.exports = function downdoc (asciidoc, { attributes: initialAttrs = {} } = {}) {
  const attrs = Object.assign(new Map(Object.entries(Object.assign({}, ATTRIBUTES, initialAttrs))), { pass: [] })
  ...
  return lines
    // biome-ignore lint/suspicious/useIterableCallbackReturn: this callback always returns
    .reduce((accum, line, idx) => {
      while (true) {
        grab = match = next = style = subs = undefined
        if (skipStack.length && (match = skipStack[skipStack.length - 1] || (line === 'endif::[]' && line))) {
          ...
          if (line === inContainer.delimiter) {
            ...
            if (inContainer.childIndent) line = inContainer.childIndent + line
          }
          ...
          const opening = inContainer === false || (line !== inContainer.delimiter && containerStack.push(inContainer))
            ...
            : undefined
          if ((grab = DELIMS[line]) === 'v' && (inContainer.verbatim = true)) {
            inContainer.subs = !(inContainer.childIndent = '') && (attrs.coseq = 1) && ['callouts']
            if (blockAttrs) {
              ...
              if ((grab = blockAttrs.get('indent')) && (grab === '0' || (inContainer.childIndent = ' '.repeat(grab)))) {
            ...
            if ((grab = blockAttrs?.get(2))) inContainer.cap = '>\n' + indent + '\u2014 ' + grab
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (inHeader) {
              ...
              if ((id = blockAttrs?.get('id'))) autoId = generateId(title).split(' ').join('-')
            } else {
              autoId = (grab = generateId((title = applySubs.call(attrs, title))).split(' ')).join('-')
            ...
            return (accum[accum.length] = line?.trimEnd()) ? accum : accum
...
function generateId (string) {
  return /[^a-z ]/.test((string = string.toLowerCase())) ? string.replace(/[^\p{Ll}\d_ -]/gu, '') : string
}
...
function image (_, target, attrlist, _idx, _str, imagesdir = this.get('imagesdir') ? this.get('imagesdir') + '/' : '') {
  const attrs = attrlist ? (~attrlist.indexOf('=') ? parseAttrlist(attrlist) :
    new Map().set(1, (~attrlist.indexOf(',') ? attrlist.substring(0, attrlist.indexOf(',')) : attrlist))): new Map()
  const img = '![' + (attrs.get(1) || /(.*\/)?(.*?)($|\.)/.exec(target)[2]) + '](' + imagesdir + target + ')'
  return attrs.has('link') ? '[' + img + '](' + attrs.get('link') + ')' : img
}
